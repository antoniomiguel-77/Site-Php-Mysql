...
<?php 
    $sql = mysqli_query($link,"SELECT * FROM tb_postagens WHERE page = 1 ORDER BY id_post DESC limit 5");
    while($line = mysqli_fetch_array($sql)){
        $titulo = $line['titulo'];
        $dt = $line['dt'];
?>
    <div class="postagens_recentes">
        <h1 class="titulos"><?php echo $titulo ?></h1>
        <span class="data" style="margin-top:-10px"><?php echo date('d-m-Y',strtotime($dt)) ?></span>
    </div>
<?php } ?>
</div>